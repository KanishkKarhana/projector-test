...
// Preserve the uploaded image's original pixel dimensions and encoding whenever
// Word supports the format. The previous implementation resized every photo to
// 900x650 and converted it to PNG, which could visibly reduce photo quality.
async function getOriginalImage(file){
  const supported = {
    'image/jpeg': {ext:'jpg', contentType:'image/jpeg'},
    'image/png':  {ext:'png', contentType:'image/png'}
  };
  if(supported[file.type]){
    return {buffer: await file.arrayBuffer(), ...supported[file.type]};
  }
  // For formats Word may not reliably support (e.g. WebP), convert at the
  // camera's native pixel dimensions without downscaling. This is only a
  // fallback; JPEG and PNG are always embedded byte-for-byte above.
  return new Promise((resolve,reject)=>{
    ...
    img.onload=()=>{
      ...
      c.toBlob(async b=>b?resolve({buffer:await b.arrayBuffer(),ext:'png',contentType:'image/png'}):reject(new Error('Could not convert image')), 'image/png');
    };
    img.onerror=()=>{URL.revokeObjectURL(url);reject(new Error(`Unsupported image format: ${file.type||'unknown'}. Please use JPG or PNG.`))};
    img.src=url;
  });
}

async function replaceDocxImage(zip, relsXml, contentTypesXml, imageNum, file){
  const image = await getOriginalImage(file);
  const oldName = `image${imageNum}.png`;
  const newName = `image${imageNum}.${image.ext}`;
  const relPattern = new RegExp(`(Target=\"media/image${imageNum}\.)png(\")`);
  if(!relPattern.test(relsXml)) throw new Error(`Could not find image${imageNum} in the Word template.`);
  relsXml = relsXml.replace(relPattern, `$1${image.ext}$2`);

  // Add the required MIME type to [Content_Types].xml if it is not already present.
  if(image.ext === 'jpg' && !contentTypesXml.includes('Extension=\"jpg\"')){
    contentTypesXml = contentTypesXml.replace('</Types>', '<Default Extension=\"jpg\" ContentType=\"image/jpeg\"/></Types>');
  }

  zip.remove(`word/media/${oldName}`);
  zip.file(`word/media/${newName}`, image.buffer);
  return {relsXml, contentTypesXml};
}
...
async function generate(){
  try{
    ...
    for(const [key,,num] of PHOTO_SLOTS){
      if(photoData[key]){
        const updated=await replaceDocxImage(zip,relsXml,contentTypesXml,num,photoData[key]);
        relsXml=updated.relsXml; contentTypesXml=updated.contentTypesXml;
      }
    }
