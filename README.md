...
Uploaded JPG/JPEG and PNG photos are now embedded into the generated DOCX using their original image bytes, without resizing or recompressing the photo itself. The Word template controls the displayed size in the report, so the report layout remains unchanged while the source image quality is preserved. Unsupported formats are converted to PNG at their native pixel dimensions as a fallback; for best results from phones/cameras, use JPG or PNG.
